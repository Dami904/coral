...
describe("formatAcpDeliverable", () => {
  it("serializes a cache_hit outcome's output as the job deliverable's output field", () => {
    const deliverable = formatAcpDeliverable({ outcome: "cache_hit", output: "high_conviction", checkedAt: "2026-01-01T00:00:00.000Z" });
    expect(JSON.parse(deliverable)).toEqual({ output: "high_conviction", note: "Conviction tier, not a safety/scam verdict." });
  });
  ...
  it("serializes a paid outcome's output the same way, dropping the tx hash from the deliverable itself", () => {
    ...
    expect(JSON.parse(deliverable)).toEqual({ output: "low_conviction", note: "Conviction tier, not a safety/scam verdict." });
  });
});
